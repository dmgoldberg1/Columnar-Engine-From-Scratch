...
static inline constexpr int64_t RowGroupSize = 32 * 1024 * 1024;
...
public:
    ...
    void WriteAll() {
        ...
        WriteNum(batch_count_, output_);
        WriteMetadata(batch_start_pos_);
        WriteNum(column_num_, output_);
        ...
        WriteNum(metadata_size_, output_);
        csv_reader_.Close();
    }
...
protected:
    void WriteGroup() {
        ...
        while (group_capacity_ <= RowGroupSize && !csv_reader_.IsEnd()) {
            ...
            int64_t row_size = csv_reader_.GetCurrRowSize(); // maybe calc it in upper func
